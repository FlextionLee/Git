...
public class HomeSaxParser {
	...
	private void loadData() {
		...
		try{
			...
			for (HomeDeal deal : deals.get(HomeDeal.APT_DEAL)) {
				for (HomeInfo home : homeInfo.values()) {
					if (deal.getAptName().contains(home.getAptName())) {
						deal.setImg(home.getImg());
						break;
					}
				}
			}			
			for (HomeDeal deal : deals.get(HomeDeal.APT_RENT)) {
				for (HomeInfo home : homeInfo.values()) {
					if (deal.getAptName().contains(home.getAptName())) {
						deal.setImg(home.getImg());
						break;
